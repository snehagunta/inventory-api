...
func TestParseHeaders(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedOperation: string(internal.OperationTypeCreated),
			expectedTxid:      "123456",
			msg: &kafka.Message{
				Headers: []kafka.Header{
					{Key: "operation", Value: []byte(string(internal.OperationTypeCreated))},
		...
		},
		{
			...
			expectedOperation: string(internal.OperationTypeUpdated),
			expectedTxid:      "123456",
			msg: &kafka.Message{
				Headers: []kafka.Header{
					{Key: "operation", Value: []byte(string(internal.OperationTypeUpdated))},
					{Key: "txid", Value: []byte("123456")},
				},
			},
			expectErr: false,
		},
		{
			...
			expectedOperation: string(internal.OperationTypeDeleted),
			expectedTxid:      "",
			msg: &kafka.Message{
				Headers: []kafka.Header{
					{Key: "operation", Value: []byte(string(internal.OperationTypeDeleted))},
					{Key: "txid", Value: []byte{}},
				},
			},
			expectErr: false,
...
func TestInventoryConsumer_ProcessMessage(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if (test.expectedOperation == string(internal.OperationTypeCreated) || test.expectedOperation == string(internal.OperationTypeUpdated)) && test.relationsEnabled {
