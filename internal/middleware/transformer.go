...
type ResourcePayload struct {
	Resource struct {
		...
		CommonResourceData string `json:"common_resource_data"`
	} `json:"resource"`
}
				...
				if ht, ok := tr.(*http.Transport); ok {
					...
					if strings.Contains(requestURI, "v1beta2") {
						...
						if method == "DELETE" {
							...
							resource, _ = createResourceRequest(payload)
						}
...
func createReporterData(reporter struct {
	...
	// Check if resource_data is valid JSON before parsing
	if strings.TrimSpace(reporter.ResourceData) != "" {
		if err := json.Unmarshal([]byte(reporter.ResourceData), &resourceData); err != nil {
			return nil, fmt.Errorf("failed to parse resource_data JSON: %w | Raw Data: %s", err, reporter.ResourceData)
		}
	}
...
func createResourceRequest(payload ResourcePayload) (*pbv2.ReportResourceRequest, error) {
	...
	// Convert reporter_data JSON string to structured data
	...
	// Convert common_resource_data JSON string to structured data
	var commonResourceData *structpb.Struct
	if strings.TrimSpace(payload.Resource.CommonResourceData) != "" {
		var commonData map[string]interface{}
		if err := json.Unmarshal([]byte(payload.Resource.CommonResourceData), &commonData); err != nil {
			return nil, fmt.Errorf("failed to parse common_resource_data JSON: %w | Raw Data: %s", err, payload.Resource.CommonResourceData)
		}
		commonResourceData, err = structpb.NewStruct(commonData)
		if err != nil {
